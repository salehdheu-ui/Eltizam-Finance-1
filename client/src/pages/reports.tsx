  ...
  return (
    <div className="space-y-4 px-1 py-4 pb-24 sm:px-2 sm:py-6 xl:px-0 xl:py-8" dir="rtl">
      <div className="flex flex-col gap-3 py-2 sm:flex-row sm:items-center sm:justify-between sm:py-4">
        ...
        <Button type="button" className="hide-on-print w-full sm:w-auto sm:self-start" onClick={() => setIsPrintPreviewOpen(true)}>
      ...
      <div className="grid grid-cols-2 gap-3 2xl:grid-cols-4">
        <Card className="bg-gradient-to-br from-emerald-50 to-emerald-100 border-emerald-200">
          <CardContent className="p-4">
            ...
            <p className="break-words text-lg font-bold text-emerald-700 sm:text-xl">+{formatCurrency(data.summary.totalIncome, 2)}</p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gradient-to-br from-red-50 to-red-100 border-red-200">
          <CardContent className="p-4">
            ...
            <p className="break-words text-lg font-bold text-red-700 sm:text-xl">-{formatCurrency(data.summary.totalExpenses, 2)}</p>
          </CardContent>
        </Card>
        ...
        <Card className={cn("border-2", data.summary.netFlow >= 0 ? "border-emerald-300 bg-emerald-50/70" : "border-red-300 bg-red-50/70")}>
          <CardContent className="p-4">
            ...
            <p className={cn("break-words text-lg font-bold sm:text-xl", data.summary.netFlow >= 0 ? "text-emerald-700" : "text-red-700")}>
        ...
        <Card className="bg-gradient-to-br from-blue-50 to-blue-100 border-blue-200">
          <CardContent className="p-4">
            ...
            <p className="break-words text-lg font-bold text-blue-700 sm:text-xl">{formatCurrency(data.summary.recurringConfiguredTotal, 2)}</p>
          </CardContent>
        </Card>
      </div>
          ...
          <div className="max-h-[75vh] overflow-y-auto bg-muted/20 p-3 sm:p-6">
            <div className="print-report-root mx-auto max-w-3xl space-y-4 rounded-[28px] bg-background p-4 sm:space-y-6 sm:rounded-3xl sm:p-8">
              <div className="print-break-avoid rounded-[28px] border bg-card p-4 sm:rounded-3xl sm:p-6">
                <div className="flex flex-col gap-3 border-b pb-4 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
                  <div className="space-y-2 text-right">
                    ...
                    <p className="text-sm text-muted-foreground">تقرير ملخص جاهز للطباعة والمشاركة</p>
                  </div>
              ...
              <div className="print-break-avoid rounded-[28px] border bg-card p-4 sm:rounded-3xl sm:p-6">
                ...
                <div className="mt-4 space-y-3">
                  {data.recentTransactions.length > 0 ? data.recentTransactions.slice(0, 8).map((tx) => (
                    <div key={`${tx.id}-print`} className="flex flex-col gap-2 rounded-2xl border p-4 text-sm sm:flex-row sm:items-center sm:justify-between">
                      <div className="min-w-0">
                        <p className="font-medium">{normalizeArabicText(tx.note) || tx.categoryName || "معاملة"}</p>
                        <p className="mt-1 text-muted-foreground">{tx.walletName || "بدون محفظة"}</p>
                      </div>
                      <span className={cn("shrink-0 font-bold", tx.type === "income" ? "text-emerald-600" : "text-red-600")}>
                        {tx.type === "income" ? "+" : "-"}{formatCurrency(tx.amount, 2)}
                      </span>
                    </div>
                  )) : <p className="text-sm text-muted-foreground">لا توجد معاملات حديثة في هذه الفترة.</p>}
                </div>
              </div>
            </div>
          </div>
